...
SELECT NOMBREPROV,CADENA,NCONSULTA FROM ((PROVEEDORES INNER JOIN CADENACONECT ON PROVEEDORES.ID = CADENACONECT.CID) INNER JOIN SQUERIES ON SQUERIES.ID = CADENACONECT.CID) WHERE CID = 2