...
CREATE TABLE SQUERIES(ID INT AUTO_INCREMENT(1,1) PRIMARY KEY NOT NULL, NCONSULTA VARCHAR(255));
CREATE TABLE TIPOSBD(TID INT AUTO_INCREMENT(1,1) PRIMARY KEY NOT NULL, NOMTIPO VARCHAR(50));
CREATE TABLE PROVEEDORES(ID INT AUTO_INCREMENT(1,1) PRIMARY KEY NOT NULL, NOMBREPROV VARCHAR(50), FKTIPO INT FOREIGN KEY REFERENCES TIPOSBD(TID););
CREATE TABLE CADENACONECT(CID INT AUTO_INCREMENT(1,1) PRIMARY KEY NOT NULL, CADENA VARCHAR(255), CADTIP INT FOREIGN KEY REFERENCES PROVEEDORES(ID););



CREATE TABLE IMPOSITORA(IID INT PRIMARY KEY AUTO_INCREMENT NOT NULL, FKPID INT FOREIGN KEY REFERENCES PROVEEDORES(ID);, CCID INT FOREIGN KEY REFERENCES CADENACONECT(CID), TDB INT REFERENCES TIPOSBD(TID));
